// ============================================================
// America250 CFC — Admin: resend the applicant confirmation email
// Admin-cookie gated. Re-sends the original "we received your submission"
// recap email for an application, without re-running AI screening or
// touching ai_screening_* / status. Useful when an applicant says they
// never got it, or it landed in spam.
// ============================================================

import { createHmac, timingSafeEqual } from "node:crypto";
import { sendConfirmationEmail, firstName } from "./screen-application.mjs";
...
export const handler = async (event) => {
  ...
  if (!SUPABASE_SERVICE_ROLE_KEY || !ADMIN_PASSWORD)
  ...
  if (typeof row.lead_email !== "string" || !EMAIL_RE.test(row.lead_email))
    return { statusCode: 422, headers: noStore, body: JSON.stringify({ error: "no_valid_email" }) };
  ...
  if (result?.error) {
    console.error("admin resend confirmation failed:", result.error);
    return { statusCode: 502, headers: noStore, body: JSON.stringify({ error: "send_failed" }) };
  }

  return { statusCode: 200, headers: noStore, body: JSON.stringify({ ok: true, sent_to: row.lead_email }) };
};
